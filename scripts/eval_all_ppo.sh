#!/bin/bash
# Evaluate all PPO checkpoints in parallel across 2 GPUs.
set -uo pipefail
...
eval_one() {
  ...
  local PPO_FINAL="$OUT/$MODEL/$DS/ppo/final"
  local OUTDIR="$OUT/$MODEL/$DS/eval_ppo"
  [[ ! -d "$PPO_FINAL" ]] && { echo "[MISSING] $MODEL/$DS — no PPO checkpoint"; return 0; }
  [[ -f "$OUTDIR/evaluation_results.json" ]] && { echo "[SKIP] $MODEL/$DS"; return 0; }
  echo "[EVAL PPO] $MODEL/$DS on GPU $GPU"
  CUDA_VISIBLE_DEVICES=$GPU $PY "$REPO/scripts/evaluate.py" \
    --model_path "$PPO_FINAL" \
    ...
    --temperature 0.8 \
    --batch_size 16 \
    > "$LOGS/${MODEL}_${DS}_eval_ppo.log" 2>&1
  echo "[DONE] $MODEL/$DS"
}
...
wait $PID0 $PID1
echo "All PPO evaluations complete."
